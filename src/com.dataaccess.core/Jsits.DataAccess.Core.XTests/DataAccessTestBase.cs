...
using Gvs.DataAccess.Core.XTests.Objects;
using System;
...
namespace Gvs.DataAccess.Core.XTests
{
    public abstract class DataAccessTestBase
    {
        ...
        public void TestExecuteStoredProcDataSet()
        {
            ...
            Assert.Single(dsUsers.Tables);
            Assert.Equal("users", dsUsers.Tables[0].TableName);
            Assert.NotEmpty(dsUsers.Tables["users"].Rows);
        }
